...
- **Minimal dependencies at runtime**: `serde` and `serde_json` (with `preserve_order`) for JSON-based compressors and agent config files, `toml_edit` for comment-preserving codex config edits, and `dialoguer` for the interactive install wizard. Dev-dependencies (`tempfile`, `tiktoken-rs`) are used for testing.
- **Rust edition 2024**.
